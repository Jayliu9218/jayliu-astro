...
- 接入 `@netlify/vite-plugin`，并加入 `vite.config.js`
- 保留并确认 `netlify/functions/api.js` 作为 `/api/*` 后端入口，数据走 Supabase
- 更新了 `README.md` 和 `NETLIFY_DEPLOY.md`，本地开发现在推荐 `npm run dev`
...
成功后会生成/更新 `dist` 文件夹。Netlify 线上部署时会自动执行这个命令，因为 `netlify.toml` 里已经配置了：
...
- 从 `netlify.toml` 移除了本地会干扰 Vite 的 catch-all redirect
- 新增 `scripts/write-spa-redirects.js`，只在生产构建后写入 `dist/_redirects`
- 更新 `package.json`，现在 `npm run build` 会执行：
...
- 新增 `scripts/start-netlify-dev.js`
- 更新 `package.json`，现在：
...
已修复 `scripts/start-netlify-dev.js` 里的 Windows 启动问题。
